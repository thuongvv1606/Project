    ...
    <body>
        ...
        <script>
            $(document).ready(function () {
                ...
                toastr.options = {
                    ...
                    "tapToDismiss": false, // Ngăn người dùng tắt toast bằng cách nhấp chuột
                    "preventDuplicates": true, // Ngăn toast trùng lặp
                    "progressBarColor": "#FF0000", // Màu của thanh tiến trình
                    "toastClass": "custom-toast" // Lớp CSS tùy chỉnh cho toast
                };
        ...
        <div class="container">
            <div class="col-sm-7">
                <form action="syllabus" method="POST">
                    <input type="text" name="key" class="btn btn-success" style="background-color: white; color: black; height: 30px; border: 2px solid #c86fff;" value="${search}">
                    <input type="submit" name="search_btn" class="btn" style="padding: 5px; letter-spacing: unset;" value="Search"><br>
                    <table>
                        <td>
                            <select name="filterByStatus">
                                <option value="-1" ${filterByStatus eq -1 ? "selected" : ""}>Status</option>
                                <option value="1" ${filterByStatus eq 1 ? "selected" : ""}>Active</option>
                                <option value="0" ${filterByStatus eq 0 ? "selected" : ""}>Dormant</option>
                            </select>
                        </td>
                        <c:if test="${sessionScope.user.getRoleFunction() == 1 || sessionScope.user.getRoleFunction() == 2}">
                            <td>
                                <select name="in_charge"">
                                    <option value="0" ${in_charge eq 0 ? "selected" : ""}>Designer</option>
                            ...
                            <td>
                                <select name="filterByApprove">
                                    <option value="-1" ${filterByApprove eq -1 ? "selected" : ""}>Approve Status</option>
                                    <option value="1" ${filterByApprove eq 1 ? "selected" : ""}>True</option>
                                    <option value="0" ${filterByApprove eq 0 ? "selected" : ""}>False</option>
                                </select>
                            </td>
                        </c:if>
                    </table>
            ...
            <div class="col-sm-5">
                <c:if test="${sessionScope.user.getRoleFunction() == 2 || sessionScope.user.getRoleFunction() == 1}">
                    <div class="row">
                        <a href="syllabus?action=add" style="margin-right: 5px;">
                            <input class="action btn" type="submit" value="Add Syllabus" name="add">
                        </a>
                        <a href="#confirmImportModal" data-toggle="modal">
                            <input class="action btn" type="submit" value="Import Syllabus" name="import">
                        </a>
                    </div>
                </c:if>
            </div>
                                ...
                                <c:forEach items="${sList}" var="c">
                                    <tr>
                                        ...
                                        <c:if test="${sessionScope.user.getRoleFunction() == 2 || sessionScope.user.getRoleFunction() == 1}">
                                            ...
                                            <td>
                                                <a href="syllabus?action=edit&sylId=${c.getSyllabusId()}">
        ...
        <div class="modal fade " id="confirmImportModal" tabindex="-1" role="dialog" aria-hidden="true" enctype="multipart/form-data">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="confirmDeleteModalLabel">Import Syllabus</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <!-- Modal body -->
                    <div class="text-dark modal-body">
                        <form action="syllabusImport" enctype="multipart/form-data" method="POST" >
                            <div class="d-flex align-items-center">
                                <a href="assets/excel/template-syllabus.xlsx" download style="text-decoration: none;color: #00a8ff">Click here to download template</a><br>
