<!-- 
    ...
    Created on : Jun 1, 2023, 8:50:48 PM
    Author     : TT
-->
    ...
    <body>
        ...
        <main>
            <div class="section-tittle text-center">
                <h2 style="margin-top: 20px;">Elective List</h2>
            </div>
            <div class="container">
                ...
                <div class="row">
                    ...
                    <div class="col-sm-6">
                        <c:if test="${sessionScope.user.getRoleFunction() == 2}">
                            <a href="elective?action=add"><button class="btn btn-primary" type="submit" style="float: right;">Add Elective</button> </a>
                        </c:if>
                    </div>
                </div>
                <h5 style="color: green" id="success">${messSuccess}</h5>
                <c:if test="${eList.size() == 0}">
                    <h4 style="color: red; text-align: center; margin: 8px;">Cannot found any electives of this curriculum!!!</h4>
                ...
                <c:if test="${eList.size() != 0}">
                    <br>
                    <div class="hint-text" style="margin-left: 20px; color: green;">Found <b>${count}</b> elective(s)</div>
                    <br>
                        ...
                        <table class="table table-striped table-hover table-sortable" style="background-color: #f9f9ff">
                            ...
                            <tbody>
                                <c:forEach items="${eList}" var="e">
                                    <tr>
                                        <td>${e.getId()}</td>
                                        <td><a href="elective?action=detail&&electiveId=${e.getId()}" style="color: blue">
                                                ${e.getCode()}</a></td>
                                        <td>${e.getName()}</a></td>
                                        <c:if test="${sessionScope.user.getRoleFunction() == 2}">
                                            <td>
                                                ${e.isActive() ? "Active" : "Dormant"}
                                            </td>
                                            <td>
                                                ...
                                                <c:if test="${e.isActive()}">
                                                    <a href="#confirmDeactiveModal${e.getId()}" data-toggle="modal">
                                                        <i style="color: red" class="fa fa-ban"></i>
                                                    </a>
                                                </c:if>
                                                <c:if test="${!e.isActive()}">
                                                    <a href="#confirmActiveModal${e.getId()}" data-toggle="modal">
                                                        <i style="color: green" class="fa fa-unlock"></i>
                                                    </a>
                                                </c:if>
                                                <div class="modal fade " id="confirmDeactiveModal${e.getId()}" tabindex="-1" role="dialog" aria-labelledby="confirmDeleteModalLabel" aria-hidden="true">
                                                    <div class="modal-dialog modal-dialog-centered" role="document">
                                                        <div class="modal-content">
                                                            ...
                                                            <div class="text-dark modal-body">
                                                                Are you sure you want to deactivate this elective?
                                                            </div>
                                                            <div class="modal-footer">
                                                                <a href="elective?action=deactivate&&electiveId=${e.getId()}"><button type="submit" class="btn btn-danger">Deactivate</button></a>
                                                                <button type="button" style="margin-right: 5px" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="modal fade " id="confirmActiveModal${e.getId()}" tabindex="-1" role="dialog" aria-labelledby="confirmDeleteModalLabel" aria-hidden="true">
                                                    <div class="modal-dialog modal-dialog-centered" role="document">
                                                        <div class="modal-content">
                                                            <div class="modal-header">
                                                            ...
                                                            <div class="text-dark modal-body">
                                                                Are you sure you want to activate this elective?
                                                            </div>
                                                            <div class="modal-footer">
                                                                <a href="elective?action=activate&&electiveId=${e.getId()}"><button type="submit" class="btn btn-danger">Activate</button></a>
                                                                <button type="button" style="margin-right: 5px" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
                                                            </div>
                                                        </div>
                                                    </div>
                    ...
                    <div class="clearfix" style="margin: 0 10px;">
                        <ul class="pagination">
                            <c:forEach begin="1" end="${end}" var="i">
                                <li class="page-item"><a href="elective?action=list&&curId=${curId}&&index=${i}" class="page-link">${i}</a></li>
                                </c:forEach>
                        </ul>
                    </div>
                </c:if>
