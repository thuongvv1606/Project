...
public class ElectiveDao extends BaseDao {
    ...
    public List<Elective> getElectiveByCurId(int id, int index, boolean check) {
        ...
        try {
            ...
            rs = ps.executeQuery();
            while (rs.next()) {
                e.add(new Elective(rs.getInt("elective_id"), rs.getString("code"), rs.getString("name"), rs.getString("description"),
    ...
    public List<Elective> searchElective(String key, int id, boolean check) {
        ...
        String query = "select * from elective "
                + "where (name like ? or code like ?) and curriculum_id = ?";
        if (check) {
    ...
    public int addElective(String code, String name, String description, int curId) {
        int generatedId = -1; // Giá trị mặc định nếu không có ID được tạo ra
        String query = "insert into elective (code, name, description, is_active, curriculum_id) \n"
                + "values (?, ?, ?, 1, ?)";
        try {
            connection = getConnection();
    ...
    public boolean checkDuplicateElective(String name, int id, int eid) {
    ...
    public Elective getElectiveById(String id) {
        ...
        try {
            ...
            rs = ps.executeQuery();
            while (rs.next()) {
                return new Elective(rs.getInt("elective_id"), rs.getString("code"), rs.getString("name"), rs.getString("description"),
                        rs.getBoolean("is_active"), new CurriculumDao().getCurriculumByID(rs.getInt("curriculum_id")),
                        new SubjectDao().getSubjectByElectiveId(rs.getInt("elective_id")));
            }
        } catch (Exception ex) {
    ...
    public void updateElective(int eid, String ecode, String ename, String edes, String status) {
        String query = "update elective set code = ?, name = ?, description = ?, is_active = ? where elective_id = ?;";
        try {
            connection = getConnection();
            ps = connection.prepareStatement(query);
    ...
    public void deleteElectiveSubject(int eid, String subject) {
        String query = "delete from elective_subject where elective_id = ? and subject_id = ?;";
        try {
            connection = getConnection();
            ps = connection.prepareStatement(query);
            ps.setInt(1, eid);
