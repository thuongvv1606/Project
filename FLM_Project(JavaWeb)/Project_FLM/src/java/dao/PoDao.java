...
public class PoDao extends BaseDao {
    ...
    public boolean checkExitstName(String poName, int curriculumId) {
        boolean check = false;
        String query = "SELECT * FROM po where name = ? and curriculum_id = ?";
        try {

            connection = getConnection();
            ps = connection.prepareStatement(query, Statement.RETURN_GENERATED_KEYS);

            ps.setString(1, poName);
            ps.setInt(2, curriculumId);
            rs = ps.executeQuery();
            if (rs.next()) {
    ...
    public void updateStatus(int id_po) {
         String sql = ""
                + "UPDATE po "
                + "SET \n"
                + "`is_active` =  0\n"
