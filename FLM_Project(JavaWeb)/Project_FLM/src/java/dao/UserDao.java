...
public class UserDao extends BaseDao {
    ...
    public boolean checkStatus(User user) {
        ...
        try {
            ...
            rs = ps.executeQuery();
            while (rs.next()) {
                return rs.getBoolean("status") == true;
    ...
    public List<User> filterUser(String key, String role, String status) {
        ArrayList<User> uList = new ArrayList<>();
        try {
            String query = "select distinct user.*, file_name from user left join avatar on user.avatar_id = avatar.avatar_id\n"
                    + "join user_role on user.user_id = user_role.user_id join role on role.role_id = user_role.role_id\n"
            ...
            ps.setString(4, "%" + key + "%");
            if (!role.equals("0")) {
                ps.setInt(5, Integer.parseInt(role));
            }
            if (!status.equals("-1")) {
                if (!role.equals("0")) {
                    ps.setInt(6, Integer.parseInt(status));
                } else {
    ...
    public int addUser(String fullname, String username, String email, String phone, String title, String company) {
        ...
        String generatedString = random.ints(leftLimit, rightLimit + 1)
                ...
                .toString();
        String encrypted = BCryptCoder.encoder(generatedString);
        String query = "INSERT INTO user"
        ...
        try {
            ...
            se.send(email, generatedString, username);
        } catch (Exception ex) {
            ex.printStackTrace();
        } finally {
            closeConnection(connection, ps, rs);
    ...
    public ArrayList<User> getAllListUser() {
        ...
        try {
            ...
            rs = ps.executeQuery();
            while (rs.next()) { 
                data.add(new User(rs.getInt("user_id"),rs.getString("full_name"), 
